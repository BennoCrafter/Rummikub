...
class PutCommand(Command):
    def execute(self, execution: Execution) -> tuple[bool, str]:
        if len(execution.command_args) < 2:
            return (
                False,
                "Not enough arguments for put command. <set id> <tile name> <position> (default to last)",
            )
        if len(execution.command_args) < 3:
            execution.command_args.append("-1")

        status, conv_command_args = args_to_right_type(
            execution.command_args, [int, str, int]
        )
        if not status:
            return (
                False,
        ...
        return (
            True,
            f"Tile {tile.colorize()} put into set {set_id} at position {position}.",
        )
    ...
    @staticmethod
    def remove_tile_from_player(player: Player, tile: Tile) -> None:
