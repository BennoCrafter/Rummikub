...
class CreateCommand(Command):
    def execute(self, execution: Execution) -> tuple[bool, str]:
        ...
        if not success:
            return (
                False,
                f"Tile {execution.command_args[len(split_tiles)]} does not exist.",
            )

        missing_tiles = self.get_missing_tiles(
            execution.game.current_player, split_tiles
        )
        if missing_tiles:
            return (
                False,
                f"You miss the following tiles: {' '.join(t.colorize() for t in missing_tiles)}",
            )
        ...
        if not tileset_instance:
            return (
                False,
                f"Invalid tileset. {' '.join(t.colorize() for t in split_tiles)}",
            )
        ...
        return (
            True,
            f"Tileset created with tiles: {' '.join(t.colorize() for t in split_tiles)}.",
        )
    ...
    @staticmethod
    def player_has_all_tiles(player: Player, tiles: list[Tile]) -> bool:
        return all(
            tile in player.tiles_cache or player.inventory.contains(tile)
            for tile in tiles
        )

    @staticmethod
    def get_missing_tiles(player: Player, tiles: list[Tile]) -> list[Tile]:
        return [
            tile
            for tile in tiles
            if tile not in player.tiles_cache and not player.inventory.contains(tile)
        ]

    @staticmethod
    def remove_tiles_from_inventory(player: Player, tiles: list[Tile]) -> None:
        for tile in tiles:
